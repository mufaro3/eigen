...
(defun load-example-data (number-securities)
  ...
  (the (list-of security)
	 ...
	 (loop :for i :in (random-sample-indices number-securities (lisp-stat:nrow directory))
	       :collect
	       (let* ((entry (lisp-stat:select directory i t))
		      ...
		      (data-filepath (if (string-equal "AGM$A" ticker-symbol)
					 (uiop:subpathname relevant-data-dir "AGM-A.csv")
					 (uiop:subpathname relevant-data-dir (format nil "~A.csv" ticker-symbol)))))
